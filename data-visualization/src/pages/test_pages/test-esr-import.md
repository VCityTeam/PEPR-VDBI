...
```js echo
const geocoded_research_structures_by_city = d3.groups(
...
```js echo
const dots = Plot.dot(geocoded_research_structures, {
  x: "longitude",
  y: "latitude",
  r: 1,
  stroke: "#f43f5e",
...
```js echo
const france_circle_domain = d3
  .geoCircle()
  .center([france_lat, france_lon])
  .radius(france_zoom)()
...
const france_city_plot = Plot.plot({
  ...
  marks: [
    ...
    Plot.geo(land, { stroke: "var(--theme-foreground-faint)" }),
    Plot.geo(borders, { stroke: "var(--theme-foreground-faint)" }),
    Plot.dot(geocoded_research_structures_by_city, {
      x: (d) => d[1][0].longitude,
      y: (d) => d[1][0].latitude,
      r: (d) => d[1].length,
      stroke: "#f43f5e",
