...
const partners_2 = extractPhase1Workbook(workbook_2).laboratories.filter(
  (d) => !partner_labels.has(d.lab),
)

partners_2.forEach((d) => partners.push({ label: d.lab }))

Promise.allSettled(
  partners.map(async (d) => {
    const label = d.label
    const response = await queryAndFormatESR(d['ID primaire'], 'aap1_export')
